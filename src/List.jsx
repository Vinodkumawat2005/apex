 ...
 const handle = () => {
  ...
  toast("Product Add Successfully !")
    ...
    for (let i = 0; i < totalpage; i++) {
        paginaction.push(
            <li key={i} className='iteam-center justify-center w-[100%]' onClick={() => Setcurrentpage(i)}>
    ...
    <Link key={index} to={`/${cate.slug}`}>
        <button
            className={`${category_slug === cate.slug ? "bg-amber-800" : "bg-gray-300"} mt-3.5 m-1.5 w-[90%] md:w-[150px] h-[40px] cursor-pointer items-center`}
        >
            {cate.name}
        </button>
    </Link>
                ...
                <div className='col-span-4 flex flex-wrap ms-[30px] me-[15px] mt-[116px] gap-3 md:col-span-4 md:ms-3.5 '>
                    {loading ? (
                        ...
                        product.map((item, index) => (
                            <div key={index} className="group  hover:shadow-xl md:ms-[50px] ms-[10px] cursor-pointer w-[230px]  relative col-span-3 mt-[26px]">
                                ...
                                <div className="mt-4 flex justify-between">
                                    ...
                                    <p onClick={handle} className="text-sm font-medium text-gray-900">${item.price}</p>
                                </div>
  ...
  handle(item);
            ...
            {/* Pagination */}
            <div className='h-[50px] mt-6 flex justify-center items-center'>
                <nav aria-label="Page navigation" className="block">
                    <ul className="inline-flex -space-x-px text-sm cursor-pointer">{paginaction}</ul>
                </nav>
            </div>
        </>
    )
